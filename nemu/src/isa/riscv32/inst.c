...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 00001 ????? 011 ????? 00100 11", seqz   , I, R(rd) = ((uint32_t)src1 < 1) ? 1 : 0);
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4));
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(rd) = src1 ^ imm); //也实现了not
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", lui    , I, R(rd) = imm);
  ...
  INSTPAT("0000000 ????? 00000 011 ????? 00100 11", snez, I, R(rd) = ((uint32_t)imm < (uint32_t)src1) ? 1 : 0); // 00000：这部分是 rd 寄存器字段。
  INSTPAT("0000111 11111 ????? 111 ????? 00100 11", zext   , I, R(rd) = src1 & imm);
