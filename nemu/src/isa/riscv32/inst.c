...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(rd) = s->pc + imm);//基于当前地址生成目标地址
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm);//构造大常量的高20位

  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = (int32_t)(int8_t)Mr(src1 + imm, 1)); //	符号扩展
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1)); // 加载一字节并扩展到32位，用 0 填充高 24 位
  INSTPAT("0000000 00001 ????? 011 ????? 00100 11", seqz   , I, R(rd) = ((uint32_t)src1 < 1) ? 1 : 0);//伪指令，sltu
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4));//字加载
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(rd) = src1 ^ imm); //也实现了not
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int32_t)src1 >> (imm & 0x1f)); // 0x1f 是一个 5 位的掩码（二进制为 11111）,作用是 提取 imm 的低 5 位，算数右移保留符号位
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (uint32_t)src1 >> (imm & 0x1f));//逻辑右移，补0
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = (uint32_t)src1 << (imm & 0x1f));//逻辑左移，补0
  INSTPAT("0000111 11111 ????? 111 ????? 00100 11", zext   , I, R(rd) = src1 & 0xFFFF);  // 与 imm（通常为 0xFFFF）掩码操作，对 src1 的字节进行零扩展
  INSTPAT("0000000 ????? ????? 111 ????? 00100 11", zext_b  , I, R(rd) = src1 & 0xFF);     // andi rd, rs1, 0xFF //伪指令andi
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = (int32_t)(int16_t)Mr(src1 + imm, 2));//加载半字
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = (uint32_t)(uint16_t)Mr(src1 + imm, 2));//无符号加载半字
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & ~1;); //& ~1 就是把目标地址的 最低位清零,riscv规定，跳转地址最低位为0

  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2)); // 将 src2 中的 32 位字存储到内存地址 src1 + imm
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));// 最低字节
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));//半字
 // 保存返回地址，更新跳转地址
  INSTPAT("??????? ????? ????? ??? 00000 11011 11", j      , J, s->dnpc = s->pc + imm); // 伪指令，jal
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (int32_t)src1 >> (src2 & 0x1f));//取低五位
  INSTPAT("0000000 ????? 00000 011 ????? 01100 11", snez   , R, R(rd) = (src1 != 0) ? 1 : 0);

  INSTPAT("0000000 00001 00000 000 00000 01100 11", ebreak, N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
