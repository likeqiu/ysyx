...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000111 11111 ????? 111 ????? 00100 11", zext, I, R(rd) = src1 & 0xFFFF);
...
//  INSTPAT("??????? ????? 00000 101 ????? 11000 11", blez, B, if ((int32_t)src2 <= 0) s->dnpc = s->pc + imm); // 允许负数比较 ,!!!这里改成src1就失败
 ...
 INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R, R(rd) = ((int64_t)(int32_t)src1 * (uint64_t)(uint32_t)src2) >> 32);
  ...
  INSTPAT("0000000 00000 00000 000 00000 00000 00", unimp, N, INV(s->pc));
  INSTPAT_END();
